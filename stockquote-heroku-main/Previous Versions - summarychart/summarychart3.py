...
fig.update_xaxes(hoverformat=hover_format, showspikes=True, spikemode="across", ticktext=ticktext, tickvals=tickvals)
fig.update_layout(yaxis=dict(side='right', range=[stock_data['Close'].min() * 0.95, stock_data['Close'].max() * 1.05]), yaxis2=dict(side='left', showticklabels=False, overlaying='y', showgrid=False, range=[0, 50*10**6]), showlegend=False)
